...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        if i == 0:
            last_layer = add_layer(input_data, units, 'rnn'+str(i))
        else:
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(GRU(units, activation='relu', return_sequences=True,
        implementation=2, name='birnn'))(input_data)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    conv_border_mode, units, recur_layers, prob=0.3, output_dim=29):
    ...
    for i in range(recur_layers):
        if i == 0:
            last_layer = add_layer(bn_cnn, units, 'rnn'+str(i))
        else:
            last_layer = add_layer(last_layer, units, 'rnn'+str(i))
